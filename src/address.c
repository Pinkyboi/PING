...
void get_dest_addr(char *host_name)
{
    ...
    if (result_ptr)
    {
        g_ping_env.dest.addr_info   = *result_ptr;
        g_ping_env.dest.sock_addr   = *result_ptr->ai_addr;
        g_ping_env.dest.bytes_addr  = ((struct sockaddr_in *)result_ptr->ai_addr)->sin_addr;
    }
